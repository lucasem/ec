...
(* basic list ops *)
let c_empty = Expr.Terminal("empty", T.l T.i, Obj.magic (ref []))
let c_concat = Expr.Terminal("concat", T.arrow (T.l T.i) (T.arrow (T.l T.i) (T.l T.i)), Lift.binary (fun a b->List.concat [a;b]))
let c_len  = Expr.Terminal("len", T.arrow (T.l T.i) T.i, Lift.unary List.length)
let c_singleton  = Expr.Terminal("singleton", T.arrow T.i (T.l T.i), Lift.unary (fun n->[n]))
...
(* patterns & finding *)
let c_find_num  = Expr.Terminal("findnum", T.arrow T.i (T.arrow (T.l T.i) T.i), Lift.binary (fun n l->
...
let c_subslice = Expr.Terminal("subslice", T.arrow T.i (T.arrow T.i (T.arrow (T.l T.i) (T.l T.i))), Lift.trinary subslice)
...
let c_replace = Expr.Terminal("replace", T.arrow T.i (T.arrow T.i (T.arrow (T.l T.i) (T.l T.i))), Lift.trinary replace)

let nth i l =
  let i = i + (if i<0 then List.length l else 0)
  in List.nth_or_default l i
let c_nth = Expr.Terminal("nth", T.arrow T.i (T.arrow (T.l T.i) T.i), Lift.binary nth)

let fnth f i l =
  let i = i + (if i<0 then List.length l else 0)
  in List.mapi ~f:(fun j v -> if j == i then get_some (f (Some(v))) else v) l
let c_fnth = Expr.Terminal("fnth", T.arrow (T.arrow T.i T.i) (T.arrow T.i (T.arrow (T.l T.i) (T.l T.i))), Lift.trinary fnth)

let map f l = List.map ~f:(fun v -> get_some (f (Some(v)))) l
let c_map = Expr.Terminal("map", T.arrow (T.arrow T.i T.i) (T.arrow (T.l T.i) (T.l T.i)), Lift.binary map)

let filter f l = List.filter ~f:(fun v -> get_some (f (Some(v)))) l
let c_filter = Expr.Terminal("filter", T.arrow (T.arrow T.i T.b) (T.arrow (T.l T.i) (T.l T.i)), Lift.binary filter)

let fold f i l = List.fold ~init:i ~f:(fun a v -> get_some (f a (Some(v)))) l
let c_fold = Expr.Terminal("fold", T.arrow (T.arrow T.i (T.arrow T.i T.i)) (T.arrow T.i (T.arrow (T.l T.i) T.i)), Lift.trinary fold)
